...
olympics.person.id= olympics.games_competitor.person_id)  
JOIN (noc_region

JOIN person_region ON olympics.noc_region.id=
olympics.person_region.region_id) ON olympics.person.id=
olympics.person_region.person_id) ON olympics.event.id=
...
olympics.competitor_event.event_id 
FROM competitor_event GROUP BY
olympics.competitor_event.event_id;)));
