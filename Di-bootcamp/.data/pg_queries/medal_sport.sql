...
from sport  JOIN (event  JOIN (person  JOIN (medal
JOIN (games_competitor  JOIN competitor_event ON olympics.games_competitor.id=
olympics.competitor_event.competitor_id) ON olympics.medal.id=
olympics.competitor_event.medal_id) ON olympics.person.id=
